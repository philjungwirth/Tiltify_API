#Similar things to the Slug_id_FundEvent_R_code can be done if you want the campaign id, to which a streamer contributed

slug <- "hasanabi"

slug_url <- paste0("https://v5api.tiltify.com/api/public/users/by/slug/", slug)
slug_response <- GET(slug_url, add_headers("Authorization" = paste("Bearer", access_token)))
slug_id <- content(slug_response)$data$id

campaigns_url <- paste0("https://v5api.tiltify.com/api/public/users/",slug_id,"/campaigns")
campaigns_response <- GET(campaigns_url, add_headers("Authorization" = paste("Bearer", access_token)))
View(content(campaigns_response))

campaigns_id <- content(campaigns_response)$data[["XYZ"]]$id #Again adjust XYZ, depending on your needs

#Here again you can use a loop similar to the one Task 4 from Slug_id_FundEvent_R_code
limit <- 10
i <- 10
